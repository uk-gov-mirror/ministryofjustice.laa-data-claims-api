 ...
 * aggregate claim/submission validation facade, not only the PDA HTTP transport. Replacing it as
 * a {@link MockitoBean} on this class means the mock is active for <em>every</em> BDD scenario,
 * not just amendment-harness scenarios. Non-amendment scenarios that depend on real
 * {@code ValidationService} behaviour (resolved-data population, area-of-law resolution, other
 * non-PDA validator side-effects) will silently see {@code valid=true} without those side-effects
 * being applied. Today's regression is green, but that green is fragile — any future scenario
 * that reads {@code resolvedData} could false-positive.
 *
 ...
 * Tracked as <b>follow-up story</b> — see
 * {@code ~/IdeaProjects/jira_drafts/DSTEW-XXXX_scope_validationservice_mock.md} for the draft
 * ticket body.
 */
...
public class CucumberSpringConfiguration {
   ...
   * Replace with a targeted PDA-transport mock in the follow-up story before adding scenarios
   * that depend on real {@code ValidationService} side-effects.
   */
