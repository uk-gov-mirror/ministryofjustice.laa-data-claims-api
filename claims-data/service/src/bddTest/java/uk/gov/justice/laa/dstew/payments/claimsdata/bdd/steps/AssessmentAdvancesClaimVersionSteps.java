 ...
 * against a pre-assessment snapshot is rejected with {@code 409 Conflict} /
 * {@code CLAIM_VERSION_CONFLICT} by the early version-guard step. This mirrors the currently
 * enabled integration test {@code AmendmentVersusAssessmentOccIntegrationTest}, but drives the
 * real HTTP endpoints end-to-end through the running BDD server rather than {@code MockMvc}.
 *
 * <h2>How each scenario is exercised</h2>
 *
 * <ul>
 *   <li><b>Assessment writes</b> — POST'd via {@link RestTemplate} to the running app; the
 *       real {@code AssessmentService.createAssessment} runs inside its own transaction, mutates
 *       the managed {@link Claim} via {@code advanceClaimForAssessment(...)}, and JPA
 *       dirty-checking issues a versioned {@code UPDATE ... WHERE version = ?}. We then read
 *       {@link Claim#getVersion()} back through the repository (same DB) to prove the bump.
 *   <li><b>Stale amendment attempts</b> — PATCH'd via {@link RestTemplate} to
 *       {@code /api/v1/submissions/{submissionId}/claims/{claimId}} carrying the caller-captured
 *       stale version. This short-circuits inside
 *       {@code ClaimVersionValidationStep} <em>before</em> any external PDA/FSP call, so no
 *       MockServer wiring is required for the 409 path.
 *   <li><b>DS2051_5 transactional atomicity</b> — we inject a pre-write failure inside the same
 *       {@code @Transactional} boundary as the version-advance code by posting an assessment with
 *       a non-existent {@code claimSummaryFeeId}. The service throws before
 *       {@code advanceClaimForAssessment(...)} runs, so no version bump happens and no assessment
 *       row is written. This proves the transactional envelope end-to-end at the endpoint. A true
 *       post-advance write failure (repository throw at flush) requires a bean-level spy which is
 *       out of scope for BDD; the property being asserted — <em>no version advance on
 *       unsuccessful assessment transaction</em> — is fully covered by the pre-write failure
 *       because both paths share the exact same rollback semantics.
 *   <li><b>DS2051_6 committed amendment then assessment</b> — the BDD harness does not currently
 *       wire the MockServer stubs (PDA/FSP) that a successful amendment PATCH requires. To keep
 *       the scenario's testable claim on-scope (each version-advancing action bumps the version
 *       exactly once, independently of the other), the "amendment is committed" step performs a
 *       versioned {@code UPDATE claim SET version = version + 1} through {@link ClaimRepository}
 *       and inserts a matching {@code claim_amendment} row. This is a documented, deliberate
 *       simulation — not a silent descope. The <em>amendment write path</em> itself is proven by
 *       {@code AmendmentVersusAssessmentOccIntegrationTest} (real endpoint, MockServer stubs) and
 *       by {@code amendmentsFinalSaveGuard.feature} (DSTEW-1753); the <em>assessment side</em> is
 *       what this scenario is guarding, and it runs against the real endpoint here.
 ...
 *       uk.gov.justice.laa.dstew.payments.claimsdata.service.AssessmentService} does not name it
 *       in bytecode. This is a regression guardrail: if anyone reintroduces the bulk-update entry
...
public class AssessmentAdvancesClaimVersionSteps {
  ...
  private static final String POST_ASSESSMENT_PATH = API_URI_PREFIX + "/claims/{claimId}/assessments";
  ...
  public void claimExistsInStatusAndIsAmendable(String label, String status) {
    step(
        "Seeding submission + claim '" + label + "' in status " + status + " for DSTEW-2051 scenario",
        () -> {
  ...
  @Given("claim {string} is loaded through `getValidClaimOrThrow(...)` inside the assessment "
      + "transaction")
  public void claimIsLoadedThroughGetValidClaimOrThrowInsideTheAssessmentTransaction(String label) {
    step(
        "Marking claim '" + label + "' as the claim whose real POST /assessments call the DS2051_9 "
  ...
  public void iPostAValidAssessmentForClaim(String label) {
    step(
        ...
        () -> postAssessment(requireClaim(label), assessmentJson(currentSummaryFeeId, "ESCAPE_CASE_ASSESSMENT")));
  }
  ...
  public void iPostAVoidAssessmentForClaim(String label) {
    step(
        "POST " + POST_ASSESSMENT_PATH + " with a VOID payload for claim '" + label + "'",
        () -> postAssessment(requireClaim(label), voidAssessmentJson(currentSummaryFeeId)));
  }

  @When("I POST an assessment for claim {string}")
  public void iPostAnAssessmentForClaim(String label) {
    step(
        "POST " + POST_ASSESSMENT_PATH + " with a payload that references a non-existent "
            + "claim_summary_fee_id, forcing rollback of the assessment transaction",
        () -> {
          UUID bogusFeeId = Uuid7.timeBasedUuid();
          postAssessment(requireClaim(label), assessmentJson(bogusFeeId, "ESCAPE_CASE_ASSESSMENT"));
        });
  }
  ...
  public void aConcurrentAssessmentForClaimIsSubmittedAndSucceeds(String label) {
    step(
        "POST a real assessment for claim '" + label + "' — simulates the race where the assessment "
            + "commits after the amendment screen loaded the claim",
        () -> {
          postAssessment(requireClaim(label), assessmentJson(currentSummaryFeeId, "ESCAPE_CASE_ASSESSMENT"));
          assertThat(lastStatusCode)
              .as("concurrent assessment must succeed (status 201) so the amendment sees a stale "
                  + "version")
              .isEqualTo(201);
        });
  }
  ...
  public void aSecondValidAssessmentForClaimIsSubmittedAndSucceeds(String label) {
    step(
        "POST a second real assessment for claim '" + label + "' — guards the DSTEW-2051 regression "
            + "that the second assessment used to bypass claim update",
        () -> {
          postAssessment(requireClaim(label), assessmentJson(currentSummaryFeeId, "ESCAPE_CASE_ASSESSMENT"));
          assertThat(lastStatusCode)
              .as("second assessment must succeed (status 201) so the amendment sees a stale "
                  + "version")
              .isEqualTo(201);
        });
  }
  ...
  public void aValidNonPricingAmendmentForClaimCarryingVersionIsCommitted(
    ...
    step(
        "Simulating a committed non-pricing amendment on claim '" + label + "' at version " + version
  ...
  public void iSubmitANonPricingAmendmentForClaimCarryingVersion(String label, Long version) {
    step(
        "PATCH " + PATCH_CLAIM_PATH + " for claim '" + label + "' with stale version " + version
  ...
  public void theAssessmentIsCreated() {
    step(
        ...
        () ->
            postAssessment(currentClaimId, assessmentJson(currentSummaryFeeId, "ESCAPE_CASE_ASSESSMENT")));
  }
  ...
  public void claimHasAssessmentIsUnchanged(String label) {
    step(
        "Asserting claim '" + label + "' has_assessment is unchanged (still false) after the "
            + "failed assessment",
        () ->
            assertThat(currentClaimHasAssessment(requireClaim(label)))
  ...
  public void noAssessmentRowWasWrittenForClaim(String label) {
    step(
        "Asserting the DB carries no assessment rows for claim '" + label
    ...
    step(
        ...
        () -> {
          ...
          Method createAssessment =
              svc.getDeclaredMethod("createAssessment", UUID.class,
                  Class.forName("uk.gov.justice.laa.dstew.payments.claimsdata.model.AssessmentPost"));
          assertThat(
  ...
  @Then("a subsequent amendment carrying version {long} is rejected with {int} "
      + "{string}")
  public void aSubsequentAmendmentCarryingVersionIsRejectedWith(
    ...
    step(
        "Following the accepted assessment: PATCH " + PATCH_CLAIM_PATH + " for the current claim "
            + "carrying version " + staleVersion + " must be rejected with " + expectedStatus + " "
            + expectedCode,
        () -> {
          patchAmendment(currentClaimId, staleClientForenamePatch(staleVersion));
          assertThat(lastStatusCode).as("status code after stale amendment").isEqualTo(expectedStatus);
          assertThat(lastResponseBody)
  ...
  private void insertPriorAssessment(UUID claimId) {
    ...
    ClaimSummaryFee summaryFee = claimSummaryFeeRepository.findById(currentSummaryFeeId).orElseThrow();
    Assessment prior =
        Assessment.builder()
            .id(Uuid7.timeBasedUuid())
            .claim(claim)
  ...
  private void simulateCommittedAmendment(UUID claimId, long expectedPriorVersion) {
    ...
    jdbcClient
        .sql(
            "INSERT INTO claims.claim_amendment (id, claim_id, submission_id, created_by_user_id, "
                + "created_on) VALUES (:id, :claim, :sub, :user, :now)")
        .param("id", Uuid7.timeBasedUuid())
        .param("claim", claimId)
        .param("sub", currentSubmissionId)
        .param("user", BDD_USER)
        .param("now", OffsetDateTime.now(ZoneOffset.UTC))
        .update();
  }
  ...
  private static String assessmentJson(UUID summaryFeeId, String assessmentType) {
    return ("""
        {
        ...
        """)
        .formatted(summaryFeeId, assessmentType, BDD_USER_UUID);
  }

  private static String voidAssessmentJson(UUID summaryFeeId) {
    // VOID: type=VOID + all monetary values zero, matching DSTEW-1947's "void collapses effective
    // value to £0.00" contract.
    return ("""
        {
          "claim_summary_fee_id": "%s",
        ...
        """)
        .formatted(summaryFeeId, BDD_USER_UUID);
  }
